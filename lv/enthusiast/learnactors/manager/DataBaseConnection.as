package lv.enthusiast.learnactors.manager {
	...
	public class DataBaseConnection {
		...
		public function initialize():void {
			...
			// check if older DB version
			result = executeQuery("SELECT version FROM tbl_version");
			if (result == null) { // version 3.1 or earlier
				executeQuery("DROP TABLE tbl_actors");
				executeQuery("DROP TABLE tbl_version");
			}
			
			if (result != null && result.data != null && result.data.length > 0) {
				if (result.data[0] == "4.0") {
					// previous vesion installed
			...
			result = executeQuery("SELECT * FROM tbl_version");
			if (result == null ) {
				createTableVersion();
			}
		}
		...
		{
			executeQuery("DROP TABLE tbl_games");
...
50.Katherine Heigl
		...
		private function createTablePacks():void {
			executeQuery("DROP TABLE tbl_packs");
			
			executeQuery("CREATE TABLE tbl_packs (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, playStoreId TEXT, appStoreId TEXT, isUnlocked INTEGER)");
			
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_025 + ", 'Top 25', '', '', 1)");
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_050 + ", 'Top 50', '', '', 0)");
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_075 + ", 'Top 75', '', '', 0)");
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_OLD + ", 'Actresses', '', '', 0)");
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_NEW + ", 'Young stars', '', '', 0)");
			executeQuery("INSERT INTO tbl_packs VALUES(" + ActorPack.PACK_ACTION + ", 'In action', '', '', 0)");
		}
		
		private function createTableVersion():void {
			executeQuery("DROP TABLE tbl_version");
			
			executeQuery("CREATE TABLE tbl_version (version TEXT )");
			
			executeQuery("INSERT INTO tbl_version VALUES('" + getAppVersion() + "')");
		}
